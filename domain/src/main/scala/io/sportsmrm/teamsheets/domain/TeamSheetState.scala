...
case object EmptyTeamSheet extends TeamSheetState {
  override def applyCommand(command: Command[?]): ReplyEffect = {
    command match {
      case CreateTeamSheet(id, date, team, opponent, replyTo) =>
        Effect.persist(TeamSheetCreated(id, date, team, opponent))
  ...
  override def applyEvent(event: Event): TeamSheetState =
    event match {
      case TeamSheetCreated(id, date, team, opponent) => CreatedTeamSheet(id, date, team, opponent)
      case _ => throw new IllegalStateException(s"unsupported event [$event] in state [EmptyTeamSheet]")
    }
}
