...
import io.sportsmrm.teamsheets.domain.TeamSheetEntitySpec.{FAREHAM_MENS_SECOND_TEAM, MENS_FIRST_TEAM, TEAM_SHEET_ID}
...
object TeamSheetEntitySpec {
  ...
  val FAREHAM_MENS_SECOND_TEAM: Opponent = Opponent(
    id = UUID.randomUUID(), displayName = "Fareham 2nd Team"
  )
  val MENS_FIRST_TEAM: Team = Team(
    id = UUID.randomUUID(), displayName = "Men's 1st Team"
  )
}

class TeamSheetEntitySpec
  extends ScalaTestWithActorTestKit(ConfigFactory.parseResources("test.conf").withFallback(EventSourcedBehaviorTestKit.config))
  with AnyWordSpecLike
  with BeforeAndAfterEach
  with LogCapturing {

  private val teamSheetTestKit = EventSourcedBehaviorTestKit[Command[?], Event, TeamSheetState](
    system,
  ...
  "A Team Sheet" when {
    "uninitialised" should {
      "handle the CreateTeamSheetCommand" in {
        ...
        val result = teamSheetTestKit.runCommand[StatusReply[Done]](CreateTeamSheet(
          id = TEAM_SHEET_ID,
          date = date,
        ...
        result.event shouldBe TeamSheetCreated(
          TEAM_SHEET_ID, date, MENS_FIRST_TEAM, FAREHAM_MENS_SECOND_TEAM,
        )
